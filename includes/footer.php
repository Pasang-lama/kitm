<footer class="custom-margin">
    <div class="container">
        <div class="row gy-4">
            <div class="col-lg-4 col-md-6 col-sm-12">
                <div class="column-heading">Kantipure Institute of Technology and Management</div>
                <div class="contact-details">
                <ul>
                    <li><i class="fa-solid fa-location-dot"></i> <span class="wrapper">Putalisadak, Kamaladi, Kathmandu Nepal</span></li>
                    <li><i class="fa-solid fa-phone"></i><span class="wrapper"><a href="">+977-123456789</a></span></li>
                    <li><i class="fa-solid fa-envelope"></i><span class="wrapper"><a href="">Company@info.com</a></span></li>
                </ul>
                </div>
            </div>
     
            <div class="col-lg-3 col-md-6 col-sm-12 ps-lg-5 ">
                <div class="column-heading">Our Resources</div>
                <ul>
                    <li><a href="page.php">Professional Training</a></li>
                    <li><a href="page.php">Inhouse Internship</a></li>
                    <li><a href="page.php">Placement Cell</a></li>
                </ul>
            </div>
            <div class="col-lg-2 col-md-6 col-sm-12">
                <div class="column-heading">Useful Links</div>
                <ul>
                    <li><a href="about-us.php">About us</a></li>
                    <li><a href="">Online Apply</a></li>
                    <li><a href="">Notices</a></li>
                    <li><a href="">News and Articles</a></li>
                    <li><a href="vaccancy.php">Vaccancy</a></li>
                    <li><a href="contact.php">Contact us</a></li>
                </ul>
            </div>
            <div class="col-lg-3 col-md-6 col-sm-12">
                <div class="column-heading">Our Programs</div>
                <ul>
                    <li><a href="bca.php">Bachelor of Computer Application (BCA)</a></li>
                    <li><a href="bcscsit.php">BSc Computer Science and Information Technology (BSc.CSIT)</a></li>
                </ul>
            </div>
        </div>
    </div>
    <div class="copy-right-section">
        <div class="container">
            <div class="wrapper">
                <p>© 2024, Kantipure Institute of Information and Technology. All Rights Reserved. Crafted by: <a href="">E-prabidhi</a>
                </p>
                <div class="privacy-policy">
                    <ul>
                        <li><a href="page.php">Policies and Procedure</a></li>
                        <li><a href="page.php">Student Code of Conduct</a></li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
</footer>
</body>



<!-- jquery  -->
<script src="js/jquery-3.6.0.min.js"></script>
<!-- bootstrap  -->
<script src="js/bootstrap.bundle.min.js"></script>
<!-- full calender  -->
<script src="js/index.global.min.js"></script>
<!-- slck slider  -->
<script src="js/slick.min.js"></script>
<!-- fancy-box  -->
<script src="js/fancybox.umd.js"></script>
<!-- mannific popup js  -->
<script src="js/jquery.magnific-popup.js"></script>
<!-- jquery timeline  plugin -->
<script src="js/horizontal_timeline.2.0.min.js"></script>
<!-- custom js file  -->
<script src="js/main.js"></script>

</html>